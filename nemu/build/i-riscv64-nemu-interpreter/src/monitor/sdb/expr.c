        ...
        if (!(0)) {
          ...
          ((void)sizeof((0) ? 1 : 0), __extension__({
             if (0)
               ...
               __assert_fail("0", "src/monitor/sdb/expr.c", 92,
...
_Bool check_parentheses(int p, int q, _Bool *status) {
  if (tokens[p].type != '(') {
    return 0;
  }
...
static int get_main_operator(int p, int q) {
  ...
  for (; p <= q; p++) {
    ...
    if ((tokens[p].type <= TK_DEREFERENCE && tokens[p].type >= TK_EQ) ||
        (tokens[p].type < 256 && tokens[p].type != '(' &&
         tokens[p].type != ')')) {
...
static expr_t eval(int p, int q, _Bool *status) {
  ...
  if (p > q) {
    *status = 0;
    ...
    if (tokens[p].type == TK_DEC) {
      val = strtol(tokens[p].str, ((void *)0), 10);
    } else if (tokens[p].type == TK_HEX) {
      val = strtol(tokens[p].str, ((void *)0), 16);
    ...
    return val;
  } else if (check_parentheses(p, q, status) == 1) {
    ...
    if (type == TK_NOT) {
      ...
      ((void)sizeof((((val2 >= (paddr_t)0x80000000 &&
                       val2 < (paddr_t)0x80000000 + 0x8000000)))
                        ? 1
                        : 0),
       __extension__({
         if (((val2 >= (paddr_t)0x80000000 &&
               val2 < (paddr_t)0x80000000 + 0x8000000)))
           ;
         else
           __assert_fail("PADDR_VALID(val2)", "src/monitor/sdb/expr.c", 257,
                         __extension__ __PRETTY_FUNCTION__);
       }));
        ...
        case '%':
          if (val2 == 0) {
            printf(
                ...
                "src/monitor/sdb/expr.c", 267, __func__);
            *status = 0;
            return 0;
          }
