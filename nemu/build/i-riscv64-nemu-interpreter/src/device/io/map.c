    ...
    default:
      ((void)sizeof((0) ? 1 : 0), __extension__({
         if (0)
           ...
           __assert_fail(
               "0", "/home/w/dev/ysyx-workbench/nemu/include/memory/host.h", 27,
    ...
    default:
      ((void)sizeof((0) ? 1 : 0), __extension__({
         if (0)
           ...
           __assert_fail(
               "0", "/home/w/dev/ysyx-workbench/nemu/include/memory/host.h", 37,
               __extension__ __PRETTY_FUNCTION__);
       }));
  }
}
...
static inline _Bool difftest_check_reg(const char *name, vaddr_t pc, word_t ref,
                                       word_t dut) {
  if (ref != dut) {
    do {
      printf(
          ...
          "x"
          "\n",
          "/home/w/dev/ysyx-workbench/nemu/include/cpu/difftest.h", 45,
...
uint8_t *new_space(int size) {
  ...
  ((void)sizeof((p_space - io_space < (2 * 1024 * 1024)) ? 1 : 0),
   __extension__({
     if (p_space - io_space < (2 * 1024 * 1024))
       ;
     else
       __assert_fail("p_space - io_space < IO_SPACE_MAX", "src/device/io/map.c",
  ...
  if (map == ((void *)0)) {
    do {
      if (!(map != ((void *)0))) {
        ...
        assert_fail_msg();
        ((void)sizeof((map != ((void *)0)) ? 1 : 0), __extension__({
           if (map != ((void *)0))
             ;
           else
             __assert_fail("map != ((void *)0)", "src/device/io/map.c", 37,
                           __extension__ __PRETTY_FUNCTION__);
         }));
      }
    } while (0);
    ...
    do {
      if (!(addr <= map->high && addr >= map->low)) {
        ...
        extern void assert_fail_msg();
        assert_fail_msg();
        ((void)sizeof((addr <= map->high && addr >= map->low) ? 1 : 0),
         __extension__({
           if (addr <= map->high && addr >= map->low)
             ;
           else
             __assert_fail("addr <= map->high && addr >= map->low",
                           "src/device/io/map.c", 39,
                           __extension__ __PRETTY_FUNCTION__);
         }));
      }
    } while (0)
...
void init_map() {
  ...
  ((void)sizeof((io_space) ? 1 : 0), __extension__({
     if (io_space)
       ;
     else
       __assert_fail("io_space", "src/device/io/map.c", 51,
                     __extension__ __PRETTY_FUNCTION__);
   }));
  p_space = io_space;
}
...
word_t map_read(paddr_t addr, int len, IOMap *map) {
  ((void)sizeof((len >= 1 && len <= 8) ? 1 : 0), __extension__({
     if (len >= 1 && len <= 8)
       ;
     else
       __assert_fail("len >= 1 && len <= 8", "src/device/io/map.c", 56,
                     __extension__ __PRETTY_FUNCTION__);
   }));
...
void map_write(paddr_t addr, int len, word_t data, IOMap *map) {
  ((void)sizeof((len >= 1 && len <= 8) ? 1 : 0), __extension__({
     if (len >= 1 && len <= 8)
       ...
       __assert_fail("len >= 1 && len <= 8", "src/device/io/map.c", 65,
                     __extension__ __PRETTY_FUNCTION__);
   }));
  check_bound(map, addr);
  paddr_t offset = addr - map->low;
