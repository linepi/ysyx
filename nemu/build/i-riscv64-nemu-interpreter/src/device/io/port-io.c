...
void add_pio_map(const char *name, ioaddr_t addr, void *space, uint32_t len,
                 io_callback_t callback) {
  ((void)sizeof((nr_map < 16) ? 1 : 0), __extension__({
     if (nr_map < 16)
       ;
     else
       __assert_fail("nr_map < NR_MAP", "src/device/io/port-io.c", 26,
                     __extension__ __PRETTY_FUNCTION__);
   }));

  ((void)sizeof((addr + len <= 65535) ? 1 : 0), __extension__({
     if (addr + len <= 65535)
       ;
     else
       __assert_fail("addr + len <= PORT_IO_SPACE_MAX",
                     "src/device/io/port-io.c", 27,
                     __extension__ __PRETTY_FUNCTION__);
   }));
...
uint32_t pio_read(ioaddr_t addr, int len) {
  ((void)sizeof((addr + len - 1 < 65535) ? 1 : 0), __extension__({
     if (addr + len - 1 < 65535)
       ;
     else
       __assert_fail("addr + len - 1 < PORT_IO_SPACE_MAX",
                     "src/device/io/port-io.c", 38,
                     __extension__ __PRETTY_FUNCTION__);
   }));
  int mapid = find_mapid_by_addr(maps, nr_map, addr);

  ((void)sizeof((mapid != -1) ? 1 : 0), __extension__({
     if (mapid != -1)
       ;
     else
       __assert_fail("mapid != -1", "src/device/io/port-io.c", 40,
                     __extension__ __PRETTY_FUNCTION__);
   }));
...
void pio_write(ioaddr_t addr, int len, uint32_t data) {
  ...
  ((void)sizeof((mapid != -1) ? 1 : 0), __extension__({
     if (mapid != -1)
       ...
       __assert_fail("mapid != -1", "src/device/io/port-io.c", 47,
                     __extension__ __PRETTY_FUNCTION__);
   }));
