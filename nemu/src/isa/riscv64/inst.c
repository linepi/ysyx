...
#define immSB() do { \
  ...
  uint32_t ut = (BITS(cht, 6, 6) << 12) | (BITS(cht, 5, 0) << 5) | (BITS(clt, 0, 0) << 11) | (BTIS(clt, 4, 1) << 1); \ 
  *imm = SEXT(ut, 12); \
} while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 110 ????? 00000 11", lwu    ,  I, R(rd) = SEXT(Mr(src1 + imm, 4), 32));
  ...
  // ============== SB ================
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , SB, s->dnpc = src1 == src2 ? s->pc + imm : s->pc);
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , SB, s->dnpc = src1 != src2 ? s->pc + imm : s->pc);
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , SB, s->dnpc = (sword_t)src1 < (sword_t)src2 ? s->pc + imm : s->pc);
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , SB, s->dnpc = (sword_t)src1 >= (sword_t)src2 ? s->pc + imm : s->pc);
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , SB, s->dnpc = src1 < src2 ? s->pc + imm : s->pc);
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , SB, s->dnpc = src1 >= src2 ? s->pc + imm : s->pc);
