...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  ,  I, printf("SEXT((%x & 0xFFFFFFFF) + %x, 32) = %x", src1, imm, SEXT((src1 & 0xFFFFFFFF) + imm, 32)); R(rd) = SEXT((src1 & 0xFFFFFFFF) + imm, 32));
  INSTPAT("0000000 ????? ????? 001 ????? 00110 11", slliw  ,  I, R(rd) = SEXT((src1 & 0xFFFFFFFF) << imm, 32));
  INSTPAT("0000000 ????? ????? 101 ????? 00110 11", srliw  ,  I, R(rd) = SEXT((uint32_t)src1 >> imm, 32));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   ,  R, R(rd) = (sword_t)((int32_t)src1 >> (int32_t)src2));
