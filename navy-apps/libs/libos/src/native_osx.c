...
static int memfd_create(const char *name, unsigned int flags) {
	int fd = glibc_open(name, flags | O_CREAT | O_RDWR);
	if (fd == -1) {
		fprintf(stderr, "memfd_create: glibc_open: %s\n", strerror(errno));
		assert(0);
	}
	if (unlink(name) != 0) {
		fprintf(stderr, "memfd_create: unlink: %s\n", strerror(errno));
		assert(0);
	}
	return fd;
}
...
void __attribute__((constructor)) init()
{
  ...
  char newpath[512];
  sprintf(newpath, "%s%s", fsimg_path, "/bin");
